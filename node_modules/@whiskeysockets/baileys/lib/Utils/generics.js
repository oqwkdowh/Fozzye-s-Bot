...
export const unixTimestampSeconds = (d = Date.now()) => Math.floor((d instanceof Date ? d.getTime() : Number(d) || Date.now()) / 1000);
...
export const printQRIfNecessaryListener = (ev) => {
...
export const fetchLatestBaileysVersion2 = async (options = {}) => {
...
export const fetchAlphaWaWebVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/wppconnect-team/wa-version/main/versions.json';
    try {
        const response = await fetch(URL, {
            dispatcher: options.dispatcher,
